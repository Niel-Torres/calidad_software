...
public class PopUpMenuEventHandle2 implements PopupMenu.OnMenuItemClickListener {
    Context context;
    View button;

    public PopUpMenuEventHandle2(Context context, View button){
    ...
    public boolean onMenuItemClick(MenuItem item) {
        ...
        if (item.getItemId()== R.id.primerItem2) {
        ...
        if (item.getItemId()==R.id.segundoItem2){
            Button b = (Button) this.button;
            CharSequence nombre = item.getTitle();
            b.setText(nombre);
            int ident = button.getId();
